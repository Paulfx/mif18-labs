...
Votre travail :
  ...
  * Cassez la section critique de 'incremente()' en ajoutant un
    'await asyncio.sleep(0)'
    cela permet de relancer l'ordonnanceur.
    Les points d'ordonnancement sont seulement sur les 'await'.
  * Pourquoi 'tictac()' est pseudo périodique et pas périodique ?
  * En supposant qu'une seule tâche prenne 100% du CPU, quelle est la durée
    maximale de retard de redémarrage d'une autre tâche ?
...
async def echo(port):
    ...
    server = await loop.create_server(echoProtocol, '127.0.0.1', port,
                                           backlog=backlog)
    await server.wait_closed()
    log("EStop")

async def client_echo(port):
    ": Un client 'echo'"
    n = information[2]
    log("Cstart{}".format(n))
    information[2] = n + 1
    while True:
        try:
            ...
            await asyncio.sleep(1) # Pour ne pas submerger le serveur
...
async def http_server():
    ...
    class HTTPProtocol(asyncio.Protocol):
        ...
        def data_received(self, data):
            ...
            if data.startswith("GET /md5 "):
                ...
                self.transport.close()
            log('Hanswer')
        async def md5(self, data):
            log('H<MD5>')
            md5 = await md5sum(data)
    ...
    log("HStart{}\n".format(http_port))
    loop = asyncio.get_event_loop()
    server = await loop.create_server(HTTPProtocol, '127.0.0.1',http_port)
    await server.wait_closed()
    log("HStop")


information = [0, 0, 0]
async def incremente():
...
async def overflow():
    ...
    await asyncio.sleep(overflow_wait)
